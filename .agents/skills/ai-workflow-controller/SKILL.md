---
name: ai-workflow-controller
description: "AI 开发流程主控调度器，负责协调 product-spec-builder、ui-prompt-generator、dev-builder、ui-ux-pro-max 等技能包之间的协作。支持指令: /prd (需求收集), /ui (生成 UI 提示词), /dev (开发代码), /run (本地运行), /check (完整度检查), /status (查看进度)。核心职责: 流程调度、状态管理、指令路由、质量控制。"
...
```python
def handle_prd():
    if not exists("Product-Spec.md"):
        mode = "0-1"
    else:
        mode = "iterative"

    result = call_skill("product-spec-builder", mode=mode)

    if result.success:
        print("产品文档已生成，下一步可以使用 /ui 或 /dev")
    else:
        print("产品文档生成失败，请重试")
```
...
def handle_ui():
    if not exists("Product-Spec.md"):
        print("错误：缺少 Product-Spec.md，请先使用 /prd 生成产品文档")
        return

    result = call_skill("ui-prompt-generator")

    if result.success:
        print("UI 提示词已生成，可以复制到 Midjourney 或 Gemini 生成设计稿")
        print("设计稿确认后，使用 /dev 开始代码开发")
    else:
        print("UI 提示词生成失败，请重试")
```
...
def handle_dev():
    ...
    result = call_skill("dev-builder")

    if result.success:
        print("功能已实现，使用 /run 启动项目")
        print("如需修改功能，先使用 /prd 更新产品文档，再使用 /dev 更新代码")
    else:
        print("功能开发失败，请重试")
```
...
def handle_run():
    if not has_project():
        print("错误：没有检测到项目，请先使用 /dev 创建项目")
        return
...
def handle_check():
    if not exists("Product-Spec.md"):
        print("错误：缺少 Product-Spec.md")
        return

    spec = read_product_spec()
    implemented = list_implemented_features()
    missing = compare_with_spec(spec, implemented)

    print("功能完整度检查：")
    print(f"已实现功能：{len(implemented)} 个")
    print(f"未实现功能：{len(missing)} 个")
...
def handle_status():
    ...
    if exists("Product-Spec.md"):
        mode = "迭代模式"
        spec = read_product_spec()
        print(f"当前模式：{mode}")
        print(f"产品文档版本：{spec.version}")
        print(f"核心功能数：{len(spec.features)}")
    else:
        mode = "0-1 模式"
        print(f"当前模式：{mode}")
        print("产品文档：未生成")
    ...
    if mode == "0-1 模式":
        ...
        print("- 使用 /ui 生成 UI 提示词")
        print("- 使用 /dev 实现新功能")
        print("- 使用 /run 运行项目")
        print("- 使用 /check 检查完整度")
