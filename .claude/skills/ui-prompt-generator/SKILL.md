...
description: "UI 提示词设计师，负责根据产品需求文档（Product-Spec.md）自动生成原型图提示词。理解需求，提炼核心功能，将功能需求转化为视觉设计描述。生成 UI-Prompts.md 文件。关键能力: 视觉转换, 风格适配, 细节描述, 多方案生成。提供 50+ 视觉风格选项，21 种配色方案，适配 Midjourney/DALL-E/Stable Diffusion 等工具。"
...
- 检查 `.claude/skills/ui-prompt-generator/templates/ui-prompt-template.md` 是否存在
- 如果模板文件不存在：
  - 立即停止执行
  - 报告错误："模板文件缺失，无法继续"
...
- 读取 `ui-prompt-template.md`，确认包含以下章节：
  - 设计风格说明
  - 配色方案
  - 核心功能提示词
  - 交互流程提示词
...
### 步骤 4：为每个核心功能生成提示词
按照 `.claude/skills/ui-prompt-generator/templates/ui-prompt-template.md` 的格式，为每个核心功能生成：
- 首页/主界面提示词
- 核心功能页面提示词（每个高优先级功能）
- 交互流程提示词（关键用户路径）
...
严格按照 `.claude/skills/ui-prompt-generator/templates/ui-prompt-template.md` 的格式生成。
...
- 依赖：必须先由 product-spec-builder 生成 Product-Spec.md
- 输入：读取 Product-Spec.md 的完整内容
- 关系：product-spec-builder 负责"想清楚"（需求），本技能负责"画出来"（设计）

### 与 dev-builder 协作
- 输出：生成 UI-Prompts.md 作为开发参考
- 顺序：在 dev-builder 之前执行
- 关系：本技能提供设计指导，dev-builder 实现代码
- **重要**：本技能只负责生成提示词，不直接实现 UI 代码

### 协作原则
- 本技能专注于"设计层"（视觉风格、配色、布局）
- dev-builder 专注于"实现层"（代码结构、交互逻辑）
- 如果用户想要直接实现 UI 代码，应使用 `/dev` 指令调用 dev-builder（可能触发 ui-ux-pro-max）

## 完成标准

- [ ] 启动前检查全部通过（模板文件存在、Product-Spec.md 完整、目录可写）
- [ ] 读取了 Product-Spec.md
- [ ] 提取了所有核心功能
- [ ] 为每个高优先级功能生成了提示词
- [ ] 提供了至少 2-3 个版本的设计方案
